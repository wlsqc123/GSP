...
void ProcessPacket(char* ptr)
{
	...
	case SC_STAT_CHANGE:
	{
		...
		if (my_packet->EXP != 0) {
			cout << my_packet->EXP << " 경험치를 얻었습니다!" << endl;
		}
		if (my_packet->HP > 0) { 
			cout << my_packet->HP << " 피해를 입혔습니다" << endl;
		}
		if (my_packet->HP < 0) {
			cout << my_packet->HP << " 피해를 입었습니다" << endl;
		}
		if (my_packet->LEVEL != 0) {
			cout << "레벨업!" << endl;
		}
		break;
...
void send_move_packet(Input key)
{
	cs_packet_move packet;
	packet.size = sizeof(packet);
	packet.type = CS_MOVE;
	packet.direction = key;
	size_t sent = 0;
	...
	while (window.isOpen())
	{
		...
		while (window.pollEvent(event))
		{
			...
			if (event.type == sf::Event::KeyPressed) {
				...
				if (None != p_type) send_move_packet(p_type);
			}
		}
