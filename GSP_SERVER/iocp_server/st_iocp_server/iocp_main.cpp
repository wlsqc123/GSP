...
constexpr int ExpMultiplier = 2;
constexpr int PowerIncrease = 10;
...
void doRecv(int key)
{
    ...
    int ret = WSARecv(objects[key].m_socket, objects[key].m_recv_over.m_wsabuf, 1,
                      nullptr, &r_flag, &objects[key].m_recv_over.m_over, nullptr);
    if (0 != ret)
    {
        int err_no = WSAGetLastError();
        if (WSA_IO_PENDING != err_no)
...
void do_move(int playerId, char dir)
{
    ...
    for (auto &id : sector[newSectorX][newSectorY].objectList_)
    {
        ...
        if (objects[id].state_ == PLST_INGAME && canSee(id, playerId))
        {
            ...
            if (true == isNpc(id))
            {
                ...
                ex_over->operaction_ = OP_PLAYER_APPROACH;
...
void calculated_damage(int attacker, int target)
{
    ...
    // 입은 데미지 전송
    sendStatChange(attacker, HP, objects[target].power_);

    // 입힌 데미지 전송
    sendStatChange(attacker, HP, objects[attacker].power_);
}
...
void calculated_exp(int attacker, int target)
{
    ...
    if (objects[target].currentHp_ <= 0)
    {
        ...
        sendStatChange(attacker, EXP, exp);
        sendRemoveObject(attacker, target);
        add_event(target, OP_RESPAWN, 30000);
    }
    ...
    while (objects[target].currentExp_ > objects[target].maxExp_)
    {
        ...
        sendStatChange(attacker, LEVEL, objects[target].level_);
    }
}
...
void process_packet(int currentPlayerId, unsigned char *packetBuffer)
{
    ...
    case CS_LOGIN:
    {
        auto packet = reinterpret_cast<cs_packet_login *>(packetBuffer);
    ...
    case CS_MOVE:
    {
        auto packet = reinterpret_cast<cs_packet_move *>(packetBuffer);
    ...
    case CS_ATTACK:
    {
        auto packet = reinterpret_cast<cs_packet_attack *>(packetBuffer);
    ...
    case CS_TELEPORT:
    {
        auto packet = reinterpret_cast<cs_packet_teleport *>(packetBuffer);
        objects[currentPlayerId].x = rand() % WORLD_WIDTH;
        objects[currentPlayerId].y = rand() % WORLD_HEIGHT;
...
{
    {
        ...
        if (objects[p_id].state_ = PLST_FREE)
            return;
...
void do_npc_random_move(S_OBJECT &npc)
{
    ...
    {
        if (PLST_INGAME != objects[o_id].state_)
            continue;
        if (true == isNpc(o_id))
            continue;
    ...
    while (true)
    {
        ...
        case OP_ACCEPT:
        {
            ...
            AcceptEx(l_socket, c_socket,
                     ex_over->packetBuffer_, 0, 32, 32, nullptr, &ex_over->m_over);
        }
        break;
        case OP_SLEEP:
...
int main()
{
    ...
    BOOL ret = AcceptEx(listenSocket, c_socket,
                        accept_over.packetBuffer_, 0, 32, 32, nullptr, &accept_over.m_over);
    if (FALSE == ret)
    {
