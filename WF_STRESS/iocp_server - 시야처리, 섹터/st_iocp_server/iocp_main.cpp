...
{
	return VIEW_RADIUS >= abs(players[id_a].x - players[id_b].x) &&
...
void do_move(int p_id, DIRECTION dir)
{
	...
	auto& old_s_x = players[p_id].s_x;
	auto& old_s_y = players[p_id].s_y;
	...
	int new_s_y = y % SECTOR_X_SIZE;

	if (new_s_x != players[p_id].s_x && new_s_y != players[p_id].s_y) {
		players[p_id].m_slock.lock();
		sector[old_s_x][old_s_y].player_list.erase(p_id);
		sector[new_s_x][new_s_y].player_list.insert(p_id);
		players[p_id].m_slock.unlock();
	}
	//else
		// 뭐해야하더라?
	...
	//for (auto& se : sector) {
	//	for (auto& se_pl : se->player_list) {
	//		if (se_pl == p_id) continue;

	//		if (players[se_pl].m_state == PLST_INGAME && can_see(se_pl, p_id))
	//			new_vl.insert(se_pl);
	//	}
	//}

	for (auto au : sector[new_s_x][new_s_y].player_list)
	{
		if (au == p_id) continue;

		if (players[au].m_state == PLST_INGAME && can_see(au, p_id))
			new_vl.insert(au);
	}
...
void process_packet(int p_id, unsigned char* p_buf)
{
	...
	case C2S_LOGIN: {
		...
		// start
		players[p_id].s_x = players[p_id].x % SECTOR_X_SIZE;
		players[p_id].s_y = players[p_id].y % SECTOR_Y_SIZE;

		players[p_id].s_mutex.lock();
		sector[players[p_id].s_x][players[p_id].s_y].player_list.insert(p_id);
		players[p_id].s_mutex.unlock();
		...
		for (auto& pl : players) {
			if (p_id != pl.id) {
				if (pl.s_x == players[p_id].s_x && pl.s_y == players[p_id].s_y)
				{
